...
type admitFunc func(*v1beta1.AdmissionRequest) ([]jsonpatch.JsonPatchOperation, error)
...
func doServeAdmitFunc(w http.ResponseWriter, r *http.Request, admit admitFunc) ([]byte, error) {
	...
	var jsonPatchOps []jsonpatch.JsonPatchOperation
	...
	if !isKubeNamespace(admissionReviewReq.Request.Namespace) {
		//patchOps, err = admit(admissionReviewReq.Request) //  <- !!!here we do the actual main function
		jsonPatchOps, err = admit(admissionReviewReq.Request) //  <- !!!here we do the actual main function
	}
	...
	if err != nil {
		...
		jsonPatchBytes, err := json.Marshal(jsonPatchOps)
		...
		admissionReviewResponse.Response.Patch = jsonPatchBytes //patchBytes
	}
